...
router.post('/reset', function (req, res) {
  if (!req.body.token || !req.body.pass) {
    return res.render(path.join(__dirname, '/../views/templates/reset.ejs'),
    {alert: true,
      alert_type: "alert-danger",
      alert_msg: "<strong> Error !</strong> Bad token."}
    );
  }

  if (!user.securePass(req.body.pass)) {
      return res.render(path.join(__dirname, '/../views/templates/reset.ejs'),
      {alert: true,
  ...
  password(req.body.pass).hash(function(error, hash) {
      if (error || !hash) {
        return res.render(path.join(__dirname, '/../views/templates/reset.ejs'),
        {alert: true,
          alert_type: "alert-danger",
          alert_msg: "<strong> Error !</strong> Password hashing error."}
        );
      }

      req.db.collection("users").update({pwdToken: req.body.token}, {$set: {password: hash}, $unset: {pwdToken: ""}}, function (err, count) {
        if (err || count.result.n < 1) {
          return res.render(path.join(__dirname, '/../views/templates/index.ejs'),
          {alert: true,
            alert_type: "alert-danger",
            alert_msg: "<strong> Error !</strong> Reset link is not valid."}
          );
        }
        ...
        {alert: true,
          ...
          alert_msg: "<strong> Success !</strong> Password updated."}
        );
      });
...
router.post('/forgot', function (req, res) {
  if (!req.body.email)  {
    ...
    {alert: true,
      alert_type: "alert-danger",
      alert_msg: "<strong> Error !</strong> Enter an email address."}
    );
  }
